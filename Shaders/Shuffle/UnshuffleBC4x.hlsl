...
void main(uint3 dtID : SV_DispatchThreadID)
{
    ...
    if(threadIndex < numQuads)
    {
        ...
        if (transformId == 3 || transformId == 19)
        {
        ...
        else  // invalid transform, simply pass through
        {
            bc4Blocks[0] = srcBuffer.Load4(firstBlockIndex * 16);
            bc4Blocks[1] = srcBuffer.Load4(firstBlockIndex * 16 + 16);
        }

        if (transformId == 19)
        {
            uint finalBlockID = ReverseSpaceCurveFor8ByteBlock(shuffledBufferSizeInBytes, widthInPixels, threadIndex * BLOCKS_PER_QUAD);
