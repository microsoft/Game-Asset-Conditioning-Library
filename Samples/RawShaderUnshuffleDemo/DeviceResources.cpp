...
void DeviceResources::CreateDeviceResources(bool forceWarp)
{
    ...
    GetAdapter(adapter.GetAddressOf(), m_forceWarp);
    ...
    m_d3dDevice->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS13, &options, sizeof(options));
    if (options.UnrestrictedBufferTextureCopyPitchSupported == false)
    {
        throw std::exception();
    }
    ...
    ThrowIfFailed(m_d3dDevice->CreateFence(0u, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(m_readbackFence.ReleaseAndGetAddressOf())));
    ...
    // Create a fence for synchronizing GPU to CPU writes.
    ThrowIfFailed(m_d3dDevice->CreateFence(0u, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(m_timerFence.ReleaseAndGetAddressOf())));
    m_timerFence->SetName(L"TimerFence");

    m_timerFenceEvent.Attach(CreateEventEx(nullptr, nullptr, 0, EVENT_MODIFY_STATE | SYNCHRONIZE));
    if (!m_timerFenceEvent.IsValid())
    {
        throw std::system_error(std::error_code(static_cast<int>(GetLastError()), std::system_category()), "CreateEventEx");
    }

    m_outputSize = RECT{ 0, 0, 1280, 720 };
}
...
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    for (UINT n = 0; n < m_backBufferCount; n++)
    {
        ...
        wchar_t name[25] = {};
        swprintf_s(name, L"BackBuffer-RT %u", n);
...
void DeviceResources::HandleDeviceLost()
{
    ...
    CreateDeviceResources(m_forceWarp);
    CreateWindowSizeDependentResources();
...
void DeviceResources::WaitForTimerFence()
{
    auto nextFenceValue = m_timerFence->GetCompletedValue() + 1;

    // Change the fence to the new value after some GPU work
    DX::ThrowIfFailed(m_commandQueue->Signal(m_timerFence.Get(), nextFenceValue));

    if (m_timerFence->GetCompletedValue() < nextFenceValue)
    {
        DX::ThrowIfFailed(m_timerFence->SetEventOnCompletion(nextFenceValue, m_timerFenceEvent.Get()));
        std::ignore = WaitForSingleObject(m_timerFenceEvent.Get(), INFINITE);
        ResetEvent(m_timerFenceEvent.Get());
    }
}
